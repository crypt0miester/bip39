(function() {
    ...
    DOM.balanceTotal = $(".total-balance");
    ...
    function getDerivationPath() {
        if (bip44TabSelected()) {
            ...
            var coin = parseIntNoNaN(DOM.bip49coin.val(), 501);
            ...
            var coin = parseIntNoNaN(DOM.bip84coin.val(), 501);
            ...
            setTimeout(function() {
                ...
                if (networks[DOM.network.val()].name == "SOL - Solana") {
                    ...
                    var path = "m/";
                        path += purpose + "'/";
                        path += coin + "'/" + index + "'";
                    var keypair = libs.solanaUtil.getKeypair(path + "/0'", seed);
                    indexText = path;
    ...
    function fetchBalance(address) {
        let result;
        fetch("https://rpc.theindex.io/mainnet-beta/4669eb75-1f53-4886-9d59-ffe83d65e865", {
            method: 'POST',
            headers: {
    ...
    function addAddressToList(indexText, address, pubkey, privkey) {
        ...
        fetch("https://rpc.theindex.io/mainnet-beta/4669eb75-1f53-4886-9d59-ffe83d65e865", {
            ...
            .then(response => {
                result = response['result']['value'] 
                if (result > 0 ){
                    balanceCell.text(`${result/1e9} SOL`);
                    DOM.balanceTotal.val(parseFloat(DOM.balanceTotal.val()) + (result/1e9))
                }
                else balanceCell.text(`${result/1e9} SOL`);
              })
    ...
    function updateCsv() {
        var tableCsv = "path,balance,address,public key,private key\n";
